...
async def get_start_func(message, strings, edit=False):
    ...
    buttons.add(
        ...
        InlineKeyboardButton(
            strings["btn_source"], url="https://t.me/IET_Owner/724"
        ),
    )
    buttons.add(
    ...
    buttons.add(
        InlineKeyboardButton(
            ...
            url=f"https://telegram.me/NasriyaManager_BoT?startgroup=true",
        )
    )
...
async def help_cmd_g(message, strings):
    ...
    button = InlineKeyboardMarkup().add(
        InlineKeyboardButton(text=text, url="https://t.me/NasriyaManager_BoT?start")
    )
    await message.reply(strings["help_header"], reply_markup=button)
